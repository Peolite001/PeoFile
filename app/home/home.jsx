...
const Home = ({ session }) => {
  ...
  const fetchPostsWithSkill = async () => {
     try{
    const postArray = [];
    const querySnapshot = await getDocs(collection(db, "peos"));
  ...
  return (
    <main className="min-h-dvh bg-gradient-to-br from-red-50 via-orange-50 to-green-50">
      <div className="flex flex-col lg:flex-row justify-center p-4 lg:p-5 gap-6 lg:gap-10">
          ...
          <div className="border border-gray-200 bg-white rounded-xl shadow-sm p-4 lg:p-6 mb-4 lg:mb-6">
            <div className="flex flex-col items-center text-center">
              <div>
                ...
                <p className="text-sm">{session?.user?.skills}</p>
              </div>
            </div>
          </div>

          <div className="border border-gray-200 bg-white rounded-xl shadow-sm p-4 lg:p-6">
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-semibold text-gray-700 text-sm lg:text-base">Connections</h3>
              <span className="bg-orange-100 text-orange-600 px-2 py-1 rounded-full text-xs lg:text-sm font-medium">0</span>
            </div>
              <p className="text-xs lg:text-sm text-gray-500">Grow your network</p>
              <Link href={"/network"}>
                <button className="w-full mt-4 bg-orange-500 text-white py-2 rounded-lg hover:bg-orange-600 transition-colors text-xs lg:text-sm font-medium">
                  Connect
                </button>
              </Link>
            </div>
        ...
        <div className="w-full lg:w-180 flex-1">
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-6">
            <div className="flex items-center gap-3 mb-3">
                <img src={session?.user?.image} alt="Profile" className="h-10 w-10 lg:h-12 lg:w-12 rounded-full border-2 border-gray-200" />
              <Link href={"/new-post"} className="flex-1">
                <div className="border rounded-full w-full h-10 py-2 px-3 cursor-pointer border-gray-300 hover:bg-gray-50 transition-colors">
                  <span className="text-gray-600 text-sm lg:text-base">Start a post</span>
                </div>
              </Link>
            </div>
          </div>
